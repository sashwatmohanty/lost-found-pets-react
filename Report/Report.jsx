...
export default function ReportingPeriodCard() {
  ...
  const handleSave = () => {
    ...
    setSuccessMessage(`Saved ${uniqueRows.length} row(s) successfully!`);
  ...
  const handleGenerate = () => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const label = `${String(periodNum).padStart(3, "0")} (${startStr} - ${endStr})`;
          ...
          >
            <div className="d-flex justify-content-between align-items-center mb-3 border-bottom pb-2">
              ...
              <div className="d-flex gap-2">
                <button onClick={() => setIsMaximized((p) => !p)} className="btn btn-sm btn-light">
                  {isMaximized ? "🗗" : "🗖"}
                </button>
                <button onClick={() => setIsOpen(false)} className="btn btn-sm btn-danger">
                  ✖
                </button>
              </div>
            </div>
            ...
            <div className="d-flex flex-wrap gap-2 align-items-center mb-3">
              <button onClick={() => setCount((c) => Math.max(0, c - 1))} className="btn btn-outline-secondary">
                -
              </button>
              ...
              <button onClick={() => setCount((c) => c + 1)} className="btn btn-outline-secondary">
                +
              </button>
